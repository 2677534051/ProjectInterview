      ...
      <van-swipe :show-indicators="false" @change="onChange" vertical :loop="false">
        <van-swipe-item v-for="(item, index) in videoList" :key="index" class="product_swiper">
          <div class="video_container">
            ...
            <!-- 播放暂停按钮 -->
            <img v-show="iconPlayShow" class="icon_play" @click="playvideo" src="../resource/icon_play.png" />
          </div>
          <!-- 右侧点赞、分享功能 -->
          <div class="tools_right">
            <div class="tools_r_li" @click="changeFollow(item, index)">
              <i class="iconfont icon-yixianshi-" :class="item.follow ? 'follow_active' : ''"></i>
          ...
          <div class="production_box">
            ...
            <div class="production_des">
              <i v-for="(list, index) in item.buyShopList" :key="index" class="production_des_i" @click="copyShopName(list.ShopUrl)">
                {{ list.ShopName }}
              </i>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    //滑动改变播放的视频
    onChange(index) {
      let inLength = this.videoList.length - 1;
      if (index == inLength) this.videoList.push(this.videoList[index]);

      //改变的时候 暂停当前播放的视频
      let video = document.querySelectorAll('video')[this.current];
      console.log();
      video.pause(this.videoList[this.current], 'this.videoList[this.current]');
